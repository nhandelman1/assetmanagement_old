...
from assetmanagement.database.popo.serviceprovider import ServiceProvider, ServiceProviderEnum
...
class BillAndDataDisplay:
    ...
    Attributes:
        ...
        bt_sp_set_dict (dict[BillType, set[ServiceProvider]]): display bills for these service
        ...
        bt_model_dict (dict[BillType, SimpleServiceModelBase]): map bill type to the model it represents
        sp_enum_bt_dict (dict[ServiceProviderEnum, BillType]): map service provider to the bill type with
            which it is associated
        re_dict (dict[int, RealEstate]): map of real estate id to a RealEstate instance with that id
        bt_sp_dict_dict (dict[BillType, dict[int, ServiceProvider]]): map of bill type to dict of service
